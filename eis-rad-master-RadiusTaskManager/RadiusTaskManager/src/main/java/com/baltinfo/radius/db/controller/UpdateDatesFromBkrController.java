...
public class UpdateDatesFromBkrController extends AbstractController {

    private static final Logger logger = LoggerFactory.getLogger(UpdateDatesFromBkrController.class);
    ...
    public List<VReductionScheduleBkr> getBkrScheduleByLotUnid(Long lotUnid) {
        EntityManager emBkr = null;
        try {
            emBkr = HibernateUtil.getInstance().getEntityManagerBKR();
            return (List<VReductionScheduleBkr>) emBkr.createQuery("SELECT s FROM VReductionScheduleBkr s " +
                    "WHERE s.lotUnid = :lotUnid")
                    .setParameter("lotUnid", lotUnid)
                    .getResultList();
        } catch (Exception ex) {
            logger.error("Error when trying to getBkrScheduleByLotUnid = {}", lotUnid, ex);
            return new ArrayList<>();
        } finally {
            closeEntityManager(emBkr);
        }
