 ...
 * @since 24.03.2023
 */
public class CianNewBuildingController extends AbstractController implements CianController<VCianRealEstate> {
    private static final Logger logger = LoggerFactory.getLogger(CianNewBuildingController.class);
    ...
    public List<VCianRealEstate> getObjectsByCategoryFeedNameAndRent(String categoryFeedCode, Boolean isRent, Long subUnid) {
        ...
        try {
            ...
            List<VCianRealEstate> list = em.createQuery("select v from VCianRealEstate v " +
                    ...
                    .getResultList();
            return list.stream()
                    .filter(v->StringUtils.equalsIgnoreCase(v.getMarketType(), CianMarketType.NEW_BUILD.getCode()))
