...
public class CarPlaceParser extends FiasAbstractParser {
    ...
    public void parse(EntityManager entityManager, String path, BiConsumer<EntityManager, List<?>> consumer) throws IOException, XMLStreamException {
        ...
        List<CarPlace> carPlaceList = new ArrayList<>();
        ...
        while (reader.hasNext()) {
            ...
            switch (event) {
                ...
                case END_ELEMENT:
                    ...
                    if (CAR_PLACE.equals(reader.getLocalName()) && carPlace.getId() != null) {
                        carPlaceList.add(carPlace);
                    }

                    if (carPlaceList.size() == BATCH_SIZE) {
                        consumer.accept(entityManager, carPlaceList);
                        carPlaceList.clear();
                    }
                    break;
            }
