 ...
 * @since 09.11.2020
 */
public class AvitoTruckClassifierJob {
    private static final Logger logger = LoggerFactory.getLogger(AvitoTruckClassifierJob.class);

    private final AvitoTruckClassificatorService avitoTruckClassificatorService;
    private final ExchangeProcController exchangeProcController;

    public AvitoTruckClassifierJob(AvitoTruckClassificatorService avitoTruckClassificatorService,
                                   ExchangeProcController exchangeProcController) {
        this.avitoTruckClassificatorService = avitoTruckClassificatorService;
        this.exchangeProcController = exchangeProcController;
    }

    @Scheduled(cron = "${job.cron.load.avito.truck.classifier}")
    public void runJob() {
        long startDate = new Date().getTime();
        logger.info("Start run Load Avito Truck Classifier Job at {}", startDate);
        Long eprUnid = null;
        try {
            Result<ExchangeProcRun, String> exchangeProcRun =
                    exchangeProcController.createExchangeProcRun(ExchangeProcs.LOAD_AVITO_TRUCK_CLASSIFIER,
                            ExchangeProcStatus.RUNNING, null);
            if (exchangeProcRun.isSuccess()) {
                eprUnid = exchangeProcRun.getResult().getEprUnid();
            }
            Result<Void, String> result = avitoTruckClassificatorService.parseTruckCatalog();
            if (result.isError()) {
            ...
            logger.error("Error when run Load Avito Truck Classifier Job", ex);
            try {
                exchangeProcController.createExchangeProcRun(ExchangeProcs.LOAD_AVITO_TRUCK_CLASSIFIER,
            ...
            logger.info("End run Load Avito Truck Classifier Job at {}; duration = {}", new Date().getTime(), (new Date().getTime() - startDate));
        }
    }
}